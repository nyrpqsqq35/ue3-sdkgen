...
inline absl::flat_hash_map<uintptr_t, std::vector<uint8_t>> ptr_to_data_map;
...
class Pointer {
 ...
 public:
  ...
  ~Pointer() {
    // the exe won't be running forever just use as much ram as we can :^)
  }
  ...
  T& operator*() const {
    if (!IsValid()) throw std::runtime_error("invalid access");
    auto& buf = ptr_to_data_map[data_];
    buf.resize(sizeof(T));
    ReadProcessMemory(process, reinterpret_cast<void*>(data_), buf.data(), buf.size(), nullptr);
    return *reinterpret_cast<T*>(ptr_to_data_map[data_].data());
  }
